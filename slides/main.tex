...
\begin{frame}{Live Demo: Nexus Sentinel}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{block}{Web Application}
        \begin{itemize}\setlength\itemsep{5pt}
          \item \textbf{Frontend}: Next.js 16 + Recharts --- deploy trên \textbf{Vercel}
          \item \textbf{Backend}: FastAPI + PyTorch --- deploy trên \textbf{Render (free)}
          \item Real-time inference: GAN generate $\to$ IDS detect
        \end{itemize}
      \end{block}
      \vspace{6pt}
      \begin{exampleblock}{Batch Mode --- Arms Race Statistics}
        \footnotesize
        \setlength{\tabcolsep}{4pt}
        \begin{tabular}{lccc}
          \toprule
          \textbf{GAN Round} & \textbf{N} & \textbf{IDS} & \textbf{Bypass Rate} \\
          \midrule
          Pure GAN (R0)   & 100 & v1 (Baseline) & 0\%     \\
          Adv. GAN (R2)   & 100 & v1 (Baseline) & \textcolor{danger}{\textbf{100\%}}   \\
          \midrule
          Pure GAN (R0)   & 100 & v2 (Hardened) & \textcolor{warning}{\textbf{$\sim$0\%}} \\
          Adv. GAN (R2)   & 100 & v2 (Hardened) & \textcolor{danger}{\textbf{100\%}}   \\
          \bottomrule
        \end{tabular}
      \end{exampleblock}
    \end{column}
    \begin{column}{0.45\textwidth}
      \begin{alertblock}{Demo URL}
        \centering\small
        \textbf{\url{https://ddos-gan.vercel.app}}\\
        \vspace{6pt}
        \footnotesize
        API Docs: \url{https://ddos-ganddos-gan-api.onrender.com/docs}
      \end{alertblock}
      \vspace{6pt}
      \begin{block}{Tính năng}
        \footnotesize
        \begin{itemize}\setlength\itemsep{3pt}
          \item \textbf{Single Payload}: Generate 1 sample $\to$ detect
          \item \textbf{Radar chart}: Visualize top 15 features
          \item \textbf{Batch Mode}: Generate N samples ($N \le 500$)
          \item \textbf{Bypass rate}: So sánh R0 vs R2 real-time
          \item \textbf{Distribution chart}: Threat score histogram
          \item Chọn v1 / v2 detector độc lập
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}[allowframebreaks]{Tài liệu tham khảo}
  ...
  \begin{thebibliography}{99}
    ...
    \bibitem{liu2022}
      G.~Liu \textit{et al.},
      ``Generating Realistic and Diverse Tests for Autonomous Vehicles,''
      \textit{IEEE TDSC}, 2022.
      (Adversarial sample generation methodology)
    ...
    \bibitem{goodfellow2018}
      ...
      \textit{OpenAI Blog}, 2018.

    \bibitem{mirsky2023}
      Y.~Mirsky \textit{et al.},
      ``The Threat of Offensive AI to Organizations,''
      \textit{Computers \& Security}, vol.~124, 2023.
...
\begin{frame}[plain]
  ...
  \vspace{12pt}
  \small\textcolor{gray}{Demo: \url{https://ddos-gan.vercel.app}}
  \vfill
\end{frame}
